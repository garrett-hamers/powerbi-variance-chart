...
/**
 * The variance chart draws several marks per category (comparison, actual, variance)
 * that share one data-source-indices key. Visual.addInteractivity promotes exactly one
 * of them to the accessibility tree as `.logical-data-point` and marks the siblings
 * aria-hidden, so screen readers announce each category once. Assert against the
 * logical points, not every painted rect.
 */
const DP_SELECTOR = `${CONTAINER_ID} svg.varianceChart .logical-data-point`;
